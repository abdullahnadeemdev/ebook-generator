...
[
  ...
  },
  {
    ...
    if (startIndex === -1 || endIndex === -1) {
      ...
      return res.status(500).json({ msg: "array not found in AI response" });
    }
    ...
    try {
      ...
      console.error("errror to parse ai response", jsonString);
      return res
        .status(500)
        .json({ msg: "Failed to generate a valid outline" });
    }
  } catch (error) {
    console.error("Error generating outline: ", error);
    res.status(500).json({ msg: "Server error during AI outline generation" });
  }
};
...
const generateChapterContent = async (req, res) => {
  try {
    ...
    if (!chapterTitle) {
      return res.status(400).json({ msg: "Please porvide a chapter title" });
    }
...
Begin writing the chapter content now:`;
    ...
    const response = await callWithRetry(() => generateOutlineAI(prompt));

    res.status(200).json({ content: response });
  } catch (error) {
    console.error("Error generating chapter content: ", error);
    res.status(500).json({
      msg: "Server error during AI chapter generation",
      err: error.message,
    });
  }
};

export { generateChapterContent, generateOutline };
